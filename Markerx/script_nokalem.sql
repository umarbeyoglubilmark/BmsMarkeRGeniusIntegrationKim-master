...
,LG_PRCLISTREF=	ISNULL((SELECT TOP 1 KF.LOGICALREF FROM LG_125_PRCLIST KF WITH(NOLOCK)  WHERE KF.BRANCH=(SELECT KDD.BRANCH FROM LK_125_DIVDEFAULTS KDD  WITH(NOLOCK) WHERE KDD.WHOUSENR=@WHOUSENR) AND KF.CARDREF=I.LOGICALREF ORDER BY KF.LOGICALREF DESC),0)
