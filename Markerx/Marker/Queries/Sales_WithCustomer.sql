...
/*REPLACE BEGIN FOR DISTINCT*/
	...
	,DOCUMENT_NO
	,DATE_=					G3B.TDATE
	,BRANCH=				(SELECT MM.LogoBranch FROM Bms_125_MarkeRGeniusIntegration_Mapping MM where PosBranch=FK_STORE)
	,CUSTOMER_CODE=			CASE WHEN CUSTOMER_CODE='' THEN (SELECT TOP 1 DD.Value FROM Bms_125_MarkeRGeniusIntegration_Default DD WHERE DD.Description='YAZARKARA FISLI CARI BOS LOGO CARISI') ELSE CUSTOMER_CODE END
	,CUSTOMER_NAME
	,POS=					FK_POS
	,FTYPE
	/*,PTYPE*/
	...
	,ITEMCODE=				ITM.CODE
	,ITEMNAME=				ITM.NAME 
	,ITEMUNIT=				(SELECT UL.CODE FROM LG_125_UNITSETL UL WHERE UL.MAINUNIT=1 AND UL.UNITSETREF=(SELECT TOP 1 I.UNITSETREF FROM LG_125_ITEMS I WITH(NOLOCK) WHERE I.CODE=ITM.CODE))
	...
	/*select * from [[BMSF_125_PennaGenius_Sales_WithCustomer]]('12/26/2023 00:00:00','12/26/2023 23:59:59') where LINE_TOTL_DISCOUNT<>0*/
/*REPLACE END FOR DISTINCT*/
FROM
	LG_125_UNITBARCODE AS UBR RIGHT OUTER JOIN
	(
...
GROUP BY cast(TRANS_DATE as date),FK_STORE,FK_POS,FK_USER, BARCODE,DOCUMENT_NO,  TRANSACTION_HEADER.CUSTOMER_CODE, TRANSACTION_HEADER.NAME_ON_DOC, TRANSACTION_HEADER.ID
	...
	LG_125_ITEMS ITM  ON ITM.LOGICALREF=UBR.ITEMREF
WHERE 
