SELECT 
LOGO_BRANCH=(SELECT top 1 MM.LogoBranch FROM Bms_124_MarkeRGeniusIntegration_Mapping MM where MM.PosBranch=STORE)
,LOGO_FICHE_TYPE=(SELECT top 1 MM.LogoFicheType FROM Bms_124_MarkeRGeniusIntegration_PaymentMapping MM where /*MM.Saleman=TF.SALESMAN AND*/ MM.IntegrationCode=TF.PAYMENT_TYPE) 
,LOGO_CURRENCY=(SELECT top 1 CASE MM.Currency WHEN 'USD' THEN '1' WHEN 'GBP' THEN '17' WHEN 'EUR' THEN '20' ELSE '0' END FROM Bms_124_MarkeRGeniusIntegration_PaymentMapping MM where /*MM.Saleman=TF.SALESMAN AND*/ MM.IntegrationCode=TF.PAYMENT_TYPE) 
,LOGO_BANK_OR_KS_CODE=(SELECT top 1 MM.BankOrKsCode FROM Bms_124_MarkeRGeniusIntegration_PaymentMapping MM where /*MM.Saleman=TF.SALESMAN AND*/ MM.IntegrationCode=TF.PAYMENT_TYPE) 
,STORE
,FTYPE
,POS
,SALESMAN
...
FROM (
	...
	,CUSTOMER_CODE= case when TH.DOCUMENT_NO<>'' AND TH.CUSTOMER_CODE='' THEN (SELECT TOP 1 DD.Value FROM Bms_124_MarkeRGeniusIntegration_Default DD WHERE DD.Description='YAZARKARA FISLI CARI BOS LOGO CARISI') ELSE CUSTOMER_CODE END
	...
	WHERE
		/*TH.PTYPE NOT IN (2, 6) AND */TH.TRANS_DATE BETWEEN @DATE1 AND @DATE2 AND TH.FK_STORE = (SELECT top 1 MM.PosBranch FROM Bms_124_MarkeRGeniusIntegration_Mapping MM where LogoBranch=0)  
    GROUP BY
           /*PYM.FK_TRANSACTION_HEADER,*/
...
GROUP BY 
	...
	,PAYMENT_TYPE_DESCRIPTION
	,SERIAL_NO
/*AND (SELECT top 1 MM.LogoFicheType FROM Bms_124_PennaGeniusIntegration_PaymentMapping MM where MM.Saleman=TF.SALESMAN AND MM.IntegrationCode=TF.PAYMENT_TYPE) IS NOT NULL*/
