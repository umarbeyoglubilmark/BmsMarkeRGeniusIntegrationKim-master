...
/*REPLACE BEGIN FOR DISTINCT*/
	...
	/*,DOCUMENT_NO*/
	,DATE_=					G3B.TDATE
	,BRANCH=				(SELECT MM.LogoBranch FROM Bms_124_ErulkuGeniusIntegration_Mapping MM where PosBranch=FK_STORE)
	,CUSTOMER_CODE
	,CUSTOMER_NAME
	,POS=					FK_POS
	,FTYPE
	...
	/*select * from [[BMSF_124_ErulkuGenius_Sales_WithCustomer]]('12/26/2023 00:00:00','12/26/2023 23:59:59') where LINE_TOTL_DISCOUNT<>0*/
/*REPLACE END FOR DISTINCT*/
FROM
	LG_124_UNITBARCODE AS UBR RIGHT OUTER JOIN
	(
SELECT FICHE_ID,FTYPE='SATIS',TDATE,FK_STORE,CUSTOMER_CODE,CUSTOMER_NAME,FK_POS,SALESMAN,BARCODE,SUM(QUANTITY) AS QUANTITY,AVG(PRICE) AS PRICE,SUM(TOTALPRICE) AS TOTALPRICE,SUM(LINE_TOTL_DISCOUNT) AS LINE_TOTL_DISCOUNT,
...
SELECT 
	...
	CUSTOMER_CODE=TRANSACTION_HEADER.CUSTOMER_CODE,CUSTOMER_NAME=TRANSACTION_HEADER.NAME_ON_DOC,
...
WHERE     ((/* satislar	*/ TRANSACTION_HEADER.PTYPE NOT IN (2, 6) AND TRANSACTION_SALE.STATUS = 0 AND TRANSACTION_HEADER.STATUS = 0) OR
                      ...
                      TRANSACTION_HEADER.TRANS_DATE BETWEEN @DATE1 AND @DATE2 AND TRANSACTION_HEADER.FK_CUSTOMER<>'0'
GROUP BY cast(TRANS_DATE as date),FK_STORE,FK_POS,FK_USER, BARCODE, TRANSACTION_HEADER.CUSTOMER_CODE, TRANSACTION_HEADER.NAME_ON_DOC, TRANSACTION_HEADER.ID
...
WHERE     (/* iptal edilen satirlar	*/ TRANSACTION_HEADER.PTYPE NOT IN (2, 6) AND TRANSACTION_SALE.STATUS = 1 AND TRANSACTION_HEADER.STATUS = 0) AND
                      TRANSACTION_HEADER.TRANS_DATE BETWEEN @DATE1 AND @DATE2 AND TRANSACTION_HEADER.FK_CUSTOMER<>'0'
GROUP BY cast(TRANS_DATE as date),FK_STORE,FK_POS,FK_USER, BARCODE, TRANSACTION_HEADER.CUSTOMER_CODE, TRANSACTION_HEADER.NAME_ON_DOC, TRANSACTION_HEADER.ID
) AS SATISVESATIRIPTAL GROUP BY FK_STORE,FK_POS,SALESMAN,TDATE,BARCODE ,CUSTOMER_CODE,CUSTOMER_NAME, FICHE_ID
UNION ALL
/*IADE*/
SELECT     
	FICHE_ID=TRANSACTION_HEADER.ID,
...
GROUP BY cast(TRANS_DATE as date),FK_STORE,FK_POS,FK_USER, BARCODE,  TRANSACTION_HEADER.CUSTOMER_CODE, TRANSACTION_HEADER.NAME_ON_DOC, TRANSACTION_HEADER.ID
	...
	LG_124_ITEMS ITM  ON ITM.LOGICALREF=UBR.ITEMREF
WHERE 
